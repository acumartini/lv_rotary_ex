...
#define CPU_HZ 48000000 // frequency that GCLK1 runs
...
void Timer::startTimer3(unsigned long period, void (*f)())
{
  ...
  // Enable IRQ
  NVIC_SetPriority(TC3_IRQn, 1); // TC3 has higher prioity than TC4
  NVIC_EnableIRQ(TC3_IRQn);

  func3 = f;

  setPeriodTimer3(period);
}

void Timer::startTimer4(unsigned long period, void (*f)())
{
  // Enable the TC bus clock, use clock generator 1
  ...
  // Enable IRQ
  NVIC_SetPriority(TC4_IRQn, 3); //TC4 has lower prioity than TC3
  NVIC_EnableIRQ(TC4_IRQn);

  func4 = f;

  setPeriodTimer4(period);
}

void Timer::setPeriodTimer3(unsigned long period)
{
  int prescaler;
...
void Timer::setPeriodTimer4(unsigned long period)
{
  int prescaler;
  uint32_t TC_CTRLA_PRESCALER_DIVN;
  ...
  // Make sure the count is in a proportional position to where it was
  // to prevent any jitter or disconnect when changing the compare value.
  TC4->COUNT16.COUNT.reg = map(TC4->COUNT16.COUNT.reg, 0, TC4->COUNT16.CC[0].reg, 0, compareValue);
